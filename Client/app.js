function getBathValue() {
  ...
  function onPageLoad() {
    ...
    //var url = "/api/get_loc_names"; // Use this if  you are using nginx. i.e tutorial 8 and onwards
    $.get(url,function(data, status) {
        console.log("got response for get_loc_names request");
        if(data) {
            var locations = data.locations;
            var uiLocations = document.getElementById("uiLocations");
            $('#uiLocations').empty();
            for(var i in locations) {
                var opt = new Option(locations[i].toString().charAt(0).toUpperCase()+locations[i].slice(1));
